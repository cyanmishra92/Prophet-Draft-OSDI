...
Critically, these routing patterns are \textbf{model-intrinsic} rather than dataset-dependent. When we train on two datasets and evaluate on a third held-out domain, prediction accuracy drops only 3--8\% compared to in-domain evaluation (\Cref{sec:implementation}). \Cref{fig:cross_domain_transfer} visualizes this robustness: when training on one domain and evaluating on others, prediction accuracy remains consistently high (69--97\%) across all 36 domain pairs, confirming that Prophet learns the model's routing structure rather than dataset-specific patterns. This robustness suggests that routing structure emerges from model architecture---how layers progressively transform representations---rather than from input semantics. The information-theoretic bounds we establish therefore apply broadly across diverse workloads, enabling deployment without per-dataset retraining.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.85\linewidth]{figs/cross_domain_transfer.pdf}
  \vspace{-10pt}
  \caption{Zero-shot cross-domain transfer accuracy on DeepSeek-MoE. Each cell shows prediction accuracy when training on one domain (row) and testing on another (column). High accuracy across all domain pairs (69--97\%) demonstrates that Prophet learns model-intrinsic expert routing patterns rather than dataset-specific features, enabling deployment without per-domain retraining.}
