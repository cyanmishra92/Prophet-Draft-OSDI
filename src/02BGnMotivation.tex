...
To assess whether experts exhibit ``hot'' and ``cold'' behavior across layers, we conducted a power-law analysis on ranked expert usage~\cite{ghorbani2022scaling}. Ranking experts by selection frequency and fitting a power-law model reveals that expert selection follows a robust power-law distribution with exponent $\alpha = 1.2$--$2.2$ across six MoE architectures (\Cref{fig:power_law}). Models with higher $\alpha$ (Qwen-30B: $\alpha=2.1$, GPT-OSS: $\alpha=2.2$) exhibit stronger concentration, while lower $\alpha$ (Qwen-2.7B, DeepSeek: $\alpha=1.2$) indicates more diverse expert usage. This heavy-tailed behavior results in extreme concentration: the top 20\% of experts account for up to 75\% of all routing decisions.

The power-law structure has two implications. First, it enables universal prefetching strategies that target consistently popular experts. Second, for throughput-oriented batched inference, power-law concentration enables batch-aware deduplication---unique experts scale sublinearly with batch size ($\mathbb{E}[U(B)] \sim B^{0.932}$), reducing bandwidth requirements. We exploit this property in our batched evaluation (\Cref{sec:implementation}).

Crucially, however, simple frequency-based methods fall short: due to temporal fluctuations and layer-specific specialization in expert popularity, naive frequency ranking yields only $\sim$12.3\% prediction accuracy, underscoring the need for cross-layer pattern modeling.
...
Beyond popularity patterns, we examine whether expert routing exhibits temporal structure across layers. Using mutual information analysis, we quantify how much knowing recent routing history reduces uncertainty about future expert selections. The key finding: cross-layer routing exhibits 0.62 bits of mutual information, meaning knowledge of recent expert selections makes future selections 54\% more predictable than random chance.

Prior systems such as FATE~\cite{fate} and AdapMoE~\cite{adapmoe} employ cross-layer information through heuristic gates or single-layer lookahead. However, these approaches lack theoretical grounding for their design choices. Our information-theoretic analysis reveals \textit{why} cross-layer context matters and \textit{how much} information each source provides.

\noindent\textbf{Information Budget.} For a model with 128 experts, maximum entropy is 7.00 bits. Our analysis reveals 4.11 bits (59\%) are exploitable for prediction, decomposed as follows:
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{Expert context from preceding 3 layers (2.59 bits, 63\%):} The sequence of recently-selected experts provides the dominant predictive signal. When a token routes through experts $e_1 \rightarrow e_2 \rightarrow e_3$ across layers, this trajectory strongly constrains which experts will be selected next.
    \item \textbf{Hidden states (0.76 bits, 18\%):} Token representations carry routing-relevant information that correlates with expert selection.
    \item \textbf{Attention patterns (0.75 bits, 18\%):} Attention weights reflect semantic structure that influences routing decisions.
\end{itemize}
...
The information budget reveals a critical insight: 63\% of exploitable information (2.59 of 4.11 bits) comes from the \textit{sequential pattern of expert selections} across layers. When a token routes through expert $e_1$ at layer $\ell-2$, then $e_2$ at layer $\ell-1$, and $e_3$ at layer $\ell$, this sequence $(e_1, e_2, e_3)$ strongly predicts which experts will be selected at layer $\ell+h$.

This makes expert prediction fundamentally a \textbf{sequence modeling problem}: given a history of expert selections, predict future selections. The state-of-the-art approach for sequence modeling is the transformer architecture, which excels at capturing dependencies across sequence elements through self-attention. We therefore design \design's predictor as a dense transformer that:
\begin{enumerate}[leftmargin=*, nosep]
    \item \textbf{Encodes expert history} as a sequence of layer-expert pairs, treating each routing decision as a token in a sequence
    \item \textbf{Learns cross-layer dependencies} through self-attention, capturing the complex patterns that determine how early-layer routing influences later-layer expert selection
    \item \textbf{Predicts multiple layers ahead} (horizon $h=3$) to provide sufficient prefetch lead time for hiding I/O latency behind computation
\end{enumerate}

This principled design---grounded in information-theoretic analysis---distinguishes \design from ad-hoc predictors that lack theoretical justification. By framing expert prediction as sequence modeling, we can achieve prediction accuracies approaching the 4.11-bit theoretical limit, enabling the proactive prefetching needed to eliminate the I/O bottleneck in MoE inference.
...
Critically, these routing patterns are \textbf{model-intrinsic} rather than dataset-dependent. To validate this claim, we evaluate cross-domain generalization: training \design on traces from one domain and testing on a completely different domain. Our six domains span diverse task types---factual question answering (Natural Questions), reading comprehension (SQuAD), mathematical reasoning (GSM8K), code generation (HumanEval), instruction following (Alpaca), and language modeling (WikiText)---ensuring that any observed generalization cannot be attributed to superficial input similarities (from here on we use the following shorthand for domain names: D1=NQ, D2=SQuAD, D3=GSM8K, D4=HumanEval, D5=Alpaca, D6=WikiText.).

\Cref{fig:cross_domain_transfer} visualizes the complete $6\times6$ transfer matrix for DeepSeek-MoE: prediction accuracy remains consistently high (67--98\%) across all 36 train-test domain pairs. Even the most challenging transfers (e.g., training on code and testing on math) achieve over 70\% accuracy. This robustness confirms that \design learns the model's internal routing structure---how transformer layers progressively transform representations---rather than dataset-specific lexical or semantic patterns.
We provide comprehensive cross-domain evaluation across all five MoE architectures in \Cref{sec:implementation}, demonstrating that this model-intrinsic property holds broadly. The information-theoretic bounds established here therefore apply across diverse workloads, enabling deployment without per-dataset retraining.
%%%%%%%%%% %%%%%%%%%% %%%%%%%%%% %%%%%%%%%% 