...
Critically, these routing patterns are \textbf{model-intrinsic} rather than dataset-dependent. When we train on two datasets and evaluate on a third held-out domain, prediction accuracy drops only 3--8\% compared to in-domain evaluation (\Cref{sec:implementation}). This robustness suggests that routing structure emerges from model architecture---how layers progressively transform representations---rather than from input semantics. The information-theoretic bounds we establish therefore apply broadly across diverse workloads, enabling deployment without per-dataset retraining.
%%%%%%%%%% %%%%%%%%%% %%%%%%%%%% %%%%%%%%%% 