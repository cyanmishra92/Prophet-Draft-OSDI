...
% \begin{itemize}[leftmargin=*, nosep]
    % \item 
    \textbf{Expert context from preceding 3 layers (63\% of exploitable):} The sequence of recently-selected experts provides the dominant predictive signal. When a token routes through experts $e_1 \rightarrow e_2 \rightarrow e_3$ across layers, this trajectory strongly constrains which experts will be selected next.
    %\item
    \textbf{Hidden states (18\% of exploitable):} Token representations carry routing-relevant information that correlates with expert selection.
    %\item
    \textbf{Attention patterns (18\% of exploitable):} Attention weights reflect semantic structure that influences routing decisions.
% \end{itemize}
...
These factors explain why the exploitable fraction ($\sim$59\%) remains consistent across architectures with 8--128 experts: the \textit{relative} reduction from power-law concentration and cross-layer dependencies is architecture-invariant, even as absolute entropy varies. The information budget reveals that 63\% of exploitable information comes from the \textit{sequential pattern of expert selections} across layers, making expert prediction fundamentally a \textbf{sequence modeling problem}.
