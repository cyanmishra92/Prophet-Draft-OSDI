...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Our design directly addresses the patterns identified in \Cref{sec:Motivation}. Temporal correlations and semantic clustering motivate neural prediction architectures that can learn complex routing dependencies across multiple MoE layers. Power-law popularity scaling enables both intelligent caching strategies that target frequently accessed experts and batch deduplication algorithms that exploit sublinear expert growth patterns. I/O dominance drives our three-pronged approach: \textbf{1.} \textbf{neural prediction} to hide memory transfer latency, \textbf{2.} \textbf{deduplication} to reduce bandwidth pressure, and \textbf{3.} \textbf{hierarchical caching} to optimize utilization of the memory hierarchy from fast GPU memory to slower system storage.

% Rather than treating expert loading as a reactive memory management problem, we transform it into a predictive optimization challenge that leverages learnable routing patterns. This fundamental shift from reactive to proactive expert management distinguishes our approach from prior work such as ExpertFlow~\cite{expertflow} and Pre-gated MoE~\cite{pregated}, which either rely on simpler heuristics or require architectural modifications. Our system operates transparently at inference time, requiring no modifications to pre-trained MoE models while providing model-agnostic acceleration across diverse architectures.

% \subsection{Design Principles and Architecture Overview}
% \label{subsec:design-principles}

% \design's architecture comprises three interconnected components that operate synergistically. The neural expert predictor processes the routing history to forecast future expert selections, exploiting the temporal and spatial structure that we identified in the expert routing patterns. The batch deduplication engine identifies and eliminates redundant expert loads across batch items, capitalizing on the power-law concentration effects where popular experts dominate routing decisions. The hierarchical cache system implements prediction-driven prefetching across three tiers optimized for different access latencies, addressing the severe memory bandwidth bottlenecks that plague current MoE deployments.

% Our design philosophy focuses on exploiting the complementary nature of the structural patterns we discovered. Temporal correlations enable multi-step prediction that provides sufficient lead time for proactive expert loading. Spatial clustering allows prediction models to leverage expert co-occurrence patterns and layer-wise specialization trends. Power-law popularity distributions create opportunities for both intelligent caching that targets frequently used experts and batch-level optimizations that exploit expert reuse across concurrent requests. These patterns collectively transform expert prefetching from an engineering heuristic into a theoretically grounded optimization problem with quantifiable benefits.
...
%For instance, \textbf{ExpertFlow} introduces a lightweight predictor to forecast routing paths before computation and then schedules expert transfers proactively—achieving substantial GPU memory savings and speedups of $2\times$--$10\times$ over baseline methods. \textbf{Pre-gated MoE}, on the other hand, redesigns the gating function to select experts for the next MoE block in advance, enabling overlapping of CPU→GPU transfers with computation and reducing latency without modifying model structure. These works take important steps toward proactive expert handling, but they either rely on architectural changes or still employ reactive mechanisms to some degree.
...
% These components work together synergistically. Temporal correlations empower multi-step expert prefetching, granting ample lead time for preloading. 
% %Spatial clustering enables the predictor to leverage patterns of expert co-occurrence and layer-wise specialization. 
% Power-law distributions act as an enabling mechanism, as caching hot experts and deduplicating batch accesses significantly reduces bandwidth demands. By bringing these elements together, \textit{Prophet} transitions expert prefetching from an engineering heuristic into a principled optimization framework with measurable benefits.
% In the following sections, we delve into the architecture of the neural predictor, detail the implementation of deduplication and hierarchical caching, and provide quantitative results that demonstrate the performance improvements enabled by our cohesive, pattern-driven design.
...
% We formalize expert prediction as a sequence modeling task where the objective is to predict future expert selections based on historical routing patterns and current hidden states. Given a token at position $t$ in layer $\ell$, we define the input as $\mathcal{H} = \{E^{(\ell-c+1:\ell)}, L^{(\ell-c+1:\ell)}, H^{(\ell)}\}$ and the output as $\hat{E}^{(\ell+h)} = \text{Predictor}(\mathcal{H}; \theta)$, where $E^{(\ell-c+1:\ell)}$ represents the expert routing history over the past $c$ layers, $L^{(\ell-c+1:\ell)}$ encodes layer positions, $H^{(\ell)}$ denotes the current hidden states that serve as router inputs, and $h$ is the prediction horizon.

% To illustrate this concretely, consider predicting expert selection for layer 8 when currently processing layer 6 (in \Cref{fig:predictor_architecture}). The predictor takes as input: the expert IDs selected for layers 4, 5, and 6 (routing history), the layer position encodings for these layers, and the hidden states from layer 6 that will be fed to the layer 8 router. The goal is to predict which expert will be selected at layer 8, providing a 3-layer prediction horizon that allows sufficient time for prefetching the predicted expert from system memory to GPU cache. The prediction horizon depends on the experts of MoE model (i.e. how many experts, how big are each experts etc.), the memory bandwidth (how long does it take to bring the said experts to GPU) and the computation window for the non-expert parts of the MoE model (used to hide the pre-fetch latency). In our case, for both switch and Qwen family models, running on consumer grade desktop machines, we observe that the prediction horizon of 3 is more than sufficient to hide the I/O latency.

% Our predictor employs a dense transformer architecture, to learn routing patterns. The architecture begins with an input embedding layer where expert identifiers are mapped to dense representations through a learned embedding matrix $\mathbf{W}_E \in \mathbb{R}^{N_E \times d}$\todo{what are teh notations}. For Switch Transformer with 128 experts, each expert ID (0-127) is mapped to a 320-dimensional dense vector, while for Qwen MoE with 64 experts, each expert ID (0-63) gets its own 320-dimensional representation. This embedding dimension balances representational capacity with computational efficiency, ensuring the predictor remains lightweight relative to the main model.

% \todo{establish a connection here -- to connect the transformer with the Dual position encoding; position of the layer}
...
\subsubsection{Neural Predictor Architecture and Training Strategy}
