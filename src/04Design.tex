...
\textit{Prophet} operates transparently at inference time, requiring no changes to pre-trained MoE models, and offers model-agnostic acceleration by unifying structural insights with learned neural predictions. Its architecture integrates two key components: a \textbf{neural expert predictor} that leverages temporal and spatial routing patterns to forecast expert needs multiple layers ahead, and a \textbf{prediction-guided hierarchical cache} that orchestrates confidence-driven expert prefetching across the memory hierarchy while exploiting the power-law popularity distribution to prioritize hot experts. These components synergistically convert expert prefetching from an engineering heuristic into a theoretically grounded optimization problem with measurable performance advantages.
...
The power-law distribution established in \Cref{sec:Motivation}---where the top 20\% of experts handle 75\% of routing decisions---motivates caching strategies that prioritize frequently-used ``hot'' experts over the long tail of rarely-activated ``cold'' experts. This insight is well-established in prior work: MoE-Infinity~\cite{moeinfinity} exploits sparsity-aware caching through Expert Activation Matrices that trace temporal locality, ProMoE~\cite{promoe} employs proactive caching with learned predictors based on token patterns, HybriMoE~\cite{zhong2025hybrimoe} introduces score-based replacement considering recency and frequency, and PreScope~\cite{prescope} combines layer-aware prediction with cross-layer scheduling and asynchronous I/O. These systems demonstrate that expert caching is essential for efficient MoE inference under memory constraints.

\noindent\textbf{Prophet's Adaptation:} Rather than designing a fundamentally new caching algorithm, \design adapts hierarchical caching to leverage its neural predictor's calibrated confidence scores. The key insight is that prediction confidence directly indicates cache placement priority: high-confidence predictions warrant L1 (GPU) placement for immediate access, while medium/lower-confidence predictions target L2 (host memory) for speculative coverage. This prediction-driven placement contrasts with reactive policies (LRU/LFU) that cannot anticipate future expert needs, and complements systems like DuoServe-MoE~\cite{duoserve} that use simpler lookahead predictors without multi-horizon confidence estimates.
...
Prophet's design directly addresses the four fundamental questions posed in \Cref{sec:01Introduction}. Our information-theoretic analysis (\Cref{sec:Motivation}) establishes that: (1) expert routing contains substantial exploitable structure through power-law distributions and cross-layer dependencies; (2) these patterns are model-intrinsic rather than data-dependent, as evidenced by 67--98\% cross-domain accuracy; (3) out-of-distribution inputs degrade prediction accuracy by only 3--8\%, confirming model-intrinsic patterns; and (4) theoretical bounds (4.11 bits of 7.00 bits exploitable) guide predictor capacity design. These findings transform expert prefetching from engineering heuristic to principled optimization with measurable performance guarantees.
...
While Prophet achieves 99.4\% effective hit rates, the end-to-end speedup of $1.5\times$--$12.7\times$ reflects the composition of cache tiers. L1 hits (sub-microsecond) contribute most to latency reduction, while L2 hits ($\sim$200$\mu$s) still incur PCIe transfer overhead---substantially less than cold misses ($\sim$2--5ms), but not zero. The speedup variance across models reflects their different I/O-to-compute ratios: models with higher I/O dominance (GPT-OSS: 92.2\%) achieve larger speedups than those with lower ratios (DeepSeek: 74.0\%).
...
Prophet adapts automatically to diverse MoE architectures. For sparse routing (top-1), precision-focused prefetching with aggressive L1 caching is most effective. For dense routing (top-4 to top-8), coverage-oriented strategies with multi-expert predictions provide better returns. The plug-and-play design requires only: (1) one-time predictor training from routing traces (3.5--4.5 hours), and (2) cache tier configuration based on available memory. No modifications to pre-trained MoE models are needed, avoiding the prohibitive cost of retraining trillion-parameter architectures.

\noindent\textbf{Complementary Approaches.}
Prophet focuses on prediction-driven prefetching rather than alternative optimization strategies. CPU/GPU co-execution systems like MoE-Lightning~\cite{cao2025moe} and kTransformers~\cite{ktransformers} exploit CPU compute resources for expert execution, achieving latency reduction through computation distribution rather than I/O hiding. These approaches are \textit{complementary} to Prophet: our prediction and caching mechanisms can enhance CPU/GPU systems by intelligently routing experts to the execution resource (CPU vs GPU) with lowest expected latency based on cache residency and prediction confidence. Similarly, Prophet's focus on expert parameters complements KV cache compression techniques~\cite{kvcache_survey}---as analyzed in \Cref{sec:background}, KV cache remains GPU-resident during inference and does not require prediction-driven prefetching, whereas expert parameters constitute the dominant I/O bottleneck.
%%%%%%%%%% %%%%%%%%%% %%%%%%%%%% %%%%%%%%%% 